...
{
    return digitalRead(PIN_SHIFT_SENSOR_Z) == LOW;
}

bool readShiftSensorCRaw()
{
    return digitalRead(PIN_SHIFT_SENSOR_C) == LOW;
}
...
void shiftInitHardwarePins()
{
    ...
    pinMode(PIN_SHIFT_SENSOR_Z, INPUT_PULLUP);
    pinMode(PIN_SHIFT_SENSOR_C, INPUT_PULLUP);
}
